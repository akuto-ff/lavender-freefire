...
app.post("/api/guilds",requireAdmin,(req,res)=>{const d=readData(),b=req.body||{},name=cleanText(b.name,50),tag=cleanText(b.tag,12).toUpperCase();if(!name||!tag)return fail(res,"Укажи название и тег");if(d.guilds.some(g=>String(g.tag).toUpperCase()===tag))return fail(res,"Такой тег уже существует");const g={id:nextId(d.guilds),name,tag,logo:cleanImage(b.logo,"🪻"),region:cleanText(b.region||"Кыргызстан",40),elo:Number(b.elo)||1200,wins:Number(b.wins)||0,losses:Number(b.losses)||0,description:cleanText(b.description,400),captain:cleanText(b.captain,50)};d.guilds.push(g);atomicWrite(d);broadcastGlobal();ok(res,g)});
app.patch("/api/guilds/:id",requireAdmin,(req,res)=>{const d=readData(),g=d.guilds.find(x=>Number(x.id)===Number(req.params.id));if(!g)return fail(res,"Гильдия не найдена",404);const b=req.body||{};if("name"in b)g.name=cleanText(b.name,50);if("tag"in b)g.tag=cleanText(b.tag,12).toUpperCase();if("logo"in b)g.logo=cleanImage(b.logo,g.logo||"🪻");if("region"in b)g.region=cleanText(b.region,40);if("elo"in b)g.elo=Number(b.elo)||0;if("wins"in b)g.wins=Number(b.wins)||0;if("losses"in b)g.losses=Number(b.losses)||0;if("description"in b)g.description=cleanText(b.description,400);if("captain"in b)g.captain=cleanText(b.captain,50);atomicWrite(d);broadcastGlobal();ok(res,g)});
app.delete("/api/guilds/:id",requireAdmin,(req,res)=>{const d=readData(),gid=Number(req.params.id);d.guilds=d.guilds.filter(g=>Number(g.id)!==gid);d.players.forEach(p=>{if(Number(p.guildId)===gid)p.guildId=null});d.matches.forEach(m=>{if(Number(m.guildAId)===gid)m.guildAId=null;if(Number(m.guildBId)===gid)m.guildBId=null});atomicWrite(d);broadcastGlobal();ok(res,{ok:true})});
app.post("/api/players",requireAdmin,(req,res)=>{const d=readData(),b=req.body||{},nickname=cleanText(b.nickname,50);if(!nickname)return fail(res,"Укажи ник игрока");const p={id:nextId(d.players),nickname,gameId:cleanText(b.gameId,40),avatar:cleanImage(b.avatar,"👤"),guildId:b.guildId?Number(b.guildId):null,elo:Number(b.elo)||1200,wins:Number(b.wins)||0,losses:Number(b.losses)||0,kills:Number(b.kills)||0,deaths:Number(b.deaths)||0,role:cleanText(b.role||"Player",30),country:cleanText(b.country||"Кыргызстан",40)};d.players.push(p);atomicWrite(d);broadcastGlobal();ok(res,p)});
app.patch("/api/players/:id",requireAdmin,(req,res)=>{const d=readData(),p=d.players.find(x=>Number(x.id)===Number(req.params.id));if(!p)return fail(res,"Игрок не найден",404);const b=req.body||{};if("nickname"in b)p.nickname=cleanText(b.nickname,50);if("gameId"in b)p.gameId=cleanText(b.gameId,40);if("avatar"in b)p.avatar=cleanImage(b.avatar,p.avatar||"👤");if("guildId"in b)p.guildId=b.guildId?Number(b.guildId):null;for(const k of ["elo","wins","losses","kills","deaths"])if(k in b)p[k]=Number(b[k])||0;atomicWrite(d);broadcastGlobal();ok(res,p)});
app.delete("/api/players/:id",requireAdmin,(req,res)=>{const d=readData(),pid=Number(req.params.id);d.players=d.players.filter(p=>Number(p.id)!==pid);d.matches.forEach(m=>{if(Number(m.playerAId)===pid)m.playerAId=null;if(Number(m.playerBId)===pid)m.playerBId=null});atomicWrite(d);broadcastGlobal();ok(res,{ok:true})});
app.post("/api/matches",requireAdmin,(req,res)=>{const d=readData(),b=req.body||{};const m={id:nextId(d.matches),tournament:cleanText(b.tournament||"LAVENDER CUP",80),title:cleanText(b.title||"LIVE MATCH",80),subtitle:cleanText(b.subtitle||"FREE FIRE",80),guildAId:b.guildAId?Number(b.guildAId):null,guildBId:b.guildBId?Number(b.guildBId):null,scoreA:Number(b.scoreA)||0,scoreB:Number(b.scoreB)||0,roundText:cleanText(b.roundText||"ROUND 1",30),status:cleanText(b.status||"SCHEDULED",20).toUpperCase(),format:cleanText(b.format||"BO7",20),playerAId:b.playerAId?Number(b.playerAId):null,playerBId:b.playerBId?Number(b.playerBId):null};d.matches.push(m);d.overlay.activeMatchId=m.id;atomicWrite(d);broadcastGlobal();ok(res,m)});
app.patch("/api/matches/:id",requireAdmin,(req,res)=>{const d=readData(),m=d.matches.find(x=>Number(x.id)===Number(req.params.id));if(!m)return fail(res,"Матч не найден",404);const b=req.body||{};for(const k of ["tournament","title","subtitle","roundText","status","format"])if(k in b)m[k]=cleanText(b[k],80);for(const k of ["guildAId","guildBId","playerAId","playerBId"])if(k in b)m[k]=b[k]?Number(b[k]):null;if("scoreA"in b)m.scoreA=Math.max(0,Number(b.scoreA)||0);if("scoreB"in b)m.scoreB=Math.max(0,Number(b.scoreB)||0);m.status=String(m.status||"LIVE").toUpperCase();atomicWrite(d);broadcastGlobal();d.streamers.forEach(s=>broadcastStreamer(s.id));ok(res,m)});
app.delete("/api/matches/:id",requireAdmin,(req,res)=>{const d=readData(),mid=Number(req.params.id);d.matches=d.matches.filter(m=>Number(m.id)!==mid);if(Number(d.overlay.activeMatchId)===mid)d.overlay.activeMatchId=d.matches[0]?.id||null;d.streamerStreams.forEach(st=>{if(Number(st.matchId)===mid)st.matchId=d.matches[0]?.id||null});atomicWrite(d);broadcastGlobal();ok(res,{ok:true})});
